...
public class Simulation {
    ...
    protected ArrayList<ChoixPersonnage> personnages = new ArrayList<>();
    protected ChoixPersonnage personnageChoisie = new ChoixPersonnage(new Point2D(WIDTH * 0.5 - 150, HEIGHT * 0.1), new Point2D(300, 381), new Image("hooke1.png"), 20, "Capitaine Rebond");
    private boolean creationPersonnage = false;
    private int indexPersonnage = 0;
    private FlecheChoixPersonnage flecheG = new FlecheChoixPersonnage(new Point2D(200, 320), new Point2D(30, 80), new Image("flecheGauche.png"));
    private FlecheChoixPersonnage flecheR = new FlecheChoixPersonnage(new Point2D(650, 320), new Point2D(30, 80), new Image("flecheDroite.png"));
    protected GoBack back = new GoBack(new Point2D(10, HEIGHT - 50), new Point2D(50, 50), new Image("Fleche.Back.png"));
    protected PersonnageQuiSaute personnageFinal;
    private Ressort ressort;
    private Planet planet;
    protected boolean creerPersonnageFinal = false;
    protected boolean utiliserAmortissement = false;
    protected ConfirmationChoixPersonnage confirmation = new ConfirmationChoixPersonnage(new Point2D(WIDTH / 2 - 68.8, HEIGHT * 3 / 4 + 30), new Point2D(137.6, 61.6), new Image("confirmation.png"));
    protected EnergyChart bc = new EnergyChart("Énergie potentielle gravitationnelle", "Énergie potentielle élastique", "Énergie cinétique", "Énergie dissipée");
    protected Slider slider = new Slider(2500, 30000, 2500);
    private String planeteChoisie = "Terre";
...
//                personnageFinal.utiliserAmortissement = utiliserAmortissement;
    ...
    public void changerPlanete(String nomPlanete) {
        ...
        switch (nomPlanete) {
            case "Terre" -> planet = new Planet(new Point2D(0,0), new Point2D(WIDTH, HEIGHT),9.81, new Image("terre.jpg"));
            case "Lune" -> planet = new Planet(new Point2D(0,0), new Point2D(WIDTH, HEIGHT), 1.62, new Image("bgLune.jpeg"));
            case "Mars" -> planet = new Planet(new Point2D(0,0), new Point2D(WIDTH, HEIGHT), 3.72, new Image("bgMars.jpeg"));
        }
    }
}
